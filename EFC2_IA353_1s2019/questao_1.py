...
def desenvolvimento():
    ...
    model.add(tf.keras.layers.Conv2D(45,kernel_size=10, activation='relu', strides=2))
#    model.add(tf.keras.layers.Conv2D(10,kernel_size=7, activation='relu'))
...
#    model.add(tf.keras.layers.Dense(256, activation=tf.nn.relu))
    ...
    model.fit(X_train, y_train, epochs=6, use_multiprocessing=True)
...
def desenvolvimento_():
    ...
    model.add(tf.keras.layers.MaxPool2D(28))
    model.add(tf.keras.layers.Flatten())
